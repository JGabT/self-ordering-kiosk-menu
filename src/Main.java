...
public class Main {
    public static void main(String[] args) {
        ...
        int discountPercent = 0; // keep track of the last applied discount
        ...
        while (true) {
            ...
            while (ordering) {
                ...
                switch (choice) {
                    case 1 -> displaySubmenu(scanner, cart, menuRepo, "Drinks", 0, 2);
                    case 2 -> displaySubmenu(scanner, cart, menuRepo, "Sides", 3, 5);
                    case 3 -> displaySubmenu(scanner, cart, menuRepo, "Burgers", 6, 8);
                    case 4 -> cart.manageCart(scanner);
                    case 5 -> {
                        scanner.nextLine(); // consume newline
                        System.out.println("Enter discount code (Valid: student10, pwd, senior): ");
    ...
    private static void displaySubmenu(Scanner scanner, Cart cart, menuRepository repo, String title, int start, int end) {
        while (true) {
            System.out.println("\n=== " + title + " ===");
            ...
            if (choice >= 1 && choice <= (end - start + 1)) {
                ...
                if (baseItem instanceof Drink) {
                    // Prompt user for size and cold option
...
//                    Consume next line
                    scanner.nextLine();
                    // Prompt for burger customizations
    ...
    private static void checkout(Scanner scanner, Cart cart, Payment payment, int dineType, int discountPercent) {
        // REDUNDANT if cart is empty before proceeding
        if (cart.getItemsInCart().isEmpty()) {
            System.out.println("\nYour cart is empty. Please add items before checking out.");
            return;
        }
...
// keeep asking until a valid payment method is entered
        while (method != 1 && method != 2) {
            ...
            if (method == 1) {
                // simulate successful cash payment
                ...
                while (!paid) {
                    ...
                    // Validate MPIN with regex: must be exactly 6 digits
                    if (mpin.matches("\\d{6}")) {
                ...
                System.out.println("Invalid payment method. Please choose 1 or 2.");
            }
        }
